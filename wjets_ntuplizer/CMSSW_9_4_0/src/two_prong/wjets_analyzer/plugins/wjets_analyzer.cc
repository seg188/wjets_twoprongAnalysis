...
class wjets_analyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      std::vector<Double_t>     fTightMuon_pt;
      std::vector<Double_t>     fTightMuon_eta;
      std::vector<Double_t>     fTightMuon_phi;
      std::vector<Double_t>     fTightMuon_mass;
...
double ftwoprong_IsolationDR(0.3);              ////////////////////CHECK 

double computeMuonIsolation(const pat::Muon * mu)
{
  double val = mu->pfIsolationR04().sumChargedHadronPt;
  if (mu->pfIsolationR04().sumNeutralHadronEt + mu->pfIsolationR04().sumPhotonEt - 0.5*mu->pfIsolationR04().sumPUPt > 0.0) 
  {
    val += mu->pfIsolationR04().sumNeutralHadronEt + mu->pfIsolationR04().sumPhotonEt - 0.5*mu->pfIsolationR04().sumPUPt > 0.0;
  }
...

{
   ...
   fTree->Branch("MET_pt", &fMET_pt, "MET_pt/D");
...
wjets_analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //STARTING ANALYSIS
    ...
    for (const pat::Muon &mu : *muons) {
      ...
      if (mu.pt() > MUON_MIN_PT &&
        ...
        computeMuonIsolation(&mu) < MUON_TIGHT_RELISO &&
  ...
  TLorentzVector fLeadingTwoProng_vector;
  if (fTwoProng_pt.size() > 0) fLeadingTwoProng_vector.SetPtEtaPhiM(fTwoProng_pt[sorted_indecies[0]], fTwoProng_eta[sorted_indecies[0]], fTwoProng_phi[sorted_indecies[0]], fTwoProng_MassPi0[sorted_indecies[0]]); 
  ...
  for (const pat::Jet &j : *jets) { //add delta r cut on muon at 0.3 
        if (j.pt() < 20) continue;
        ...
        TLorentzVector temp_jet_vector;
        temp_jet_vector.SetPtEtaPhiM(j.pt(), j.eta(), j.phi(), j.mass());

        fHT_bare += j.pt();

        if (temp_jet_vector.DeltaR(fMuon_vector) < 0.3 or temp_jet_vector.DeltaR(fLeadingTwoProng_vector) < 0.3) continue;
